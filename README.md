...
`DISAMBIGUATE_ESC_CODES` (`1`), `REPORT_ALL_KEYS_AS_ESC_CODES` (`8`), and
`REPORT_ASSOCIATED_TEXT` (`16`) across native and browser input paths. Flag `1`
disambiguates combinations such as `Ctrl-I` without changing legacy `Enter`,
`Tab`, and `Backspace`; flag `8` also reports text keys and those legacy C0
keys as CSI-u; flag `16` adds safe associated text codepoints when flag `8` is
active. Kitty graphics/image protocols are not part of this support. See

...
if [[ -f "${LOCAL_OPENGL_ONLY_MARKER}" && "${WITTY_ALLOW_LOCAL_CHROMIUM_SMOKE:-}" != "1" ]]; then
  cat >&2 <<'MSG'
Witty browser smoke is disabled on this machine by .witty-local-opengl-only.
...
if ! command -v node >/dev/null 2>&1; then
  echo "missing node; cannot run Playwright smoke" >&2
  exit 2
fi
...
WITTY_PLAYWRIGHT_ROOT="${PLAYWRIGHT_ROOT}" node "${ROOT_DIR}/scripts/run-witty-web-smoke.mjs"
